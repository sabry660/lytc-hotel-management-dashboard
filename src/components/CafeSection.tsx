...
export default function CafeSection({ orders: initialOrders = [], onUpdateOrderStatus }: CafeSectionProps) {
  ...
  const loadPendingOrders = async () => {
    try {
      const response = await apiService.getCafePendingOrders();
  ...
  const loadMenu = async () => {
    try {
      const response = await apiService.getCafeMenu(0, 50);
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 border-b border-gray-900 pb-5">
        <div>
          <h1 className="text-2xl font-black text-[#E6C587] flex items-center gap-2">
            <Coffee size={24} className="text-[#D4AF37]" />
            المقهى
          </h1>
          <p className="text-gray-500 text-xs mt-1">إدارة طلبات المقهى والقائمة</p>
        </div>
        <button 
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <div className="p-4 bg-[#090909] border border-gray-900 rounded-xl hover:border-emerald-500/35 transition duration-200">
          <div className="space-y-1">
            ...
            <div className="text-lg font-bold text-[#E6C587] font-mono">{totalCafeSales.toLocaleString('ar-SA')}</div>
          </div>
      ...
      {viewMode === 'orders' && (
        <>
          {isLoading ? (
            ...
            <div className="text-center py-16 bg-[#0b0b0b] border border-gray-900 rounded-2xl">
              <Coffee size={48} className="text-gray-700 mx-auto mb-4" />
                    ...
                    <div className="flex justify-between items-start mb-4">
                      <div className="flex items-center gap-2">
                        <div className="w-10 h-10 bg-[#D4AF37]/20 border border-[#D4AF37]/30 rounded-lg flex items-center justify-center">
                          <Coffee size={18} className="text-[#E6C587]" />
                        </div>
                        <div>
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-gray-800">
                ...
                <th className="text-xs text-gray-500 font-bold text-right pb-3">الطاولة</th>
            ...
            <tbody>
              {pendingOrders.map((order: any) => (
                <tr key={order.orderId} className="border-b border-gray-800/50 hover:bg-[#121212]/50 transition-colors">
                  ...
                  <td className="py-3 text-sm text-white">{order.tableNumber}</td>
                  ...
                  <td className="py-3">
                    <div className="flex items-center gap-2">
                      <button
                        onClick={() => apiService.updateCafeOrderStatus(order.orderId, 'COMPLETED')}
                        className="p-1.5 bg-emerald-950/20 border border-emerald-500/30 rounded-lg hover:bg-emerald-900/30 transition"
                      >
                        <Check size={14} className="text-emerald-400" />
                      </button>
                      <button
                        onClick={() => apiService.updateCafeOrderStatus(order.orderId, 'CANCELLED')}
